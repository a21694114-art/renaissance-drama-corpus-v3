...
def main():
    ...
    mask_info = json.load(open(ch / 'name_mask_summary.json')) if (ch / 'name_mask_summary.json').exists() else {}
    masked_run = bool(mask_info) and emb_info.get('text_sha256') and emb_info.get('text_sha256') == mask_info.get('masked_sha256')   # the embeddings really were computed on the masked text
    scheme_rows = []
    for cand in [runs / 'compare_schemes.csv'] + sorted(runs.parent.glob('runs_*_masked/compare_schemes.csv')):
        if cand.exists(): scheme_rows = list(csv.DictReader(open(cand, encoding='utf-8'))); break
...
window.addEventListener('resize',()=>{if(lb.classList.contains('on')&&!lb.classList.contains('big')){sc=fitScale();apply();}});})();"""
    ...
    hd = (run_info.get('settings') or {}).get('hdbscan') or {}
    dd = run_info.get('dedup_editions') or {}
    n_fit = run_info.get('n_in_fit'); n_placed = (run_info.get('n_chunks') or 0) - (n_fit or 0)
    use_counts = Counter(cat_of(t) for t in topics)
    mask_html = ''
    if mask_info:
        mask_html = ('<h2>Name masking</h2><p>Character names were replaced by the placeholder <code>%s</code> before embedding, play by play, using the cast lists of Kim\'s character-clustering project matched to each edition by its TCP id '
                     '(%s editions matched exactly, %s through the collection volume\'s base id, %s by an explicit override; %s editions without a cast list — pageants and entertainments — were left unmasked). '
                     'Only persons are masked: places, nations and demonyms, deities, role words (king, duke, nurse) and personifications stay. A name that is also a common word is masked only when capitalised; spelling variants of a cast name within the edition (I/J, u/v, similarity ≥ %s) are caught. '
                     '%s of %s tokens were replaced (%.1f %%). Keywords are computed on the unmasked text, so names can appear in a topic\'s keyword list although the embedding never saw them. Nicknames absent from the cast lists (Harry, Jack) survive the masking and may still link passages.</p>'
                     % (E(str(mask_info.get('placeholder', 'someone'))), f"{mask_info.get('matched_exact', 0):,}", f"{mask_info.get('matched_base', 0):,}", f"{mask_info.get('matched_override', 0):,}", f"{mask_info.get('unmatched', 0):,}",
                        mask_info.get('variant_sim', 0.85), f"{mask_info.get('tokens_replaced', 0):,}", f"{mask_info.get('tokens_total', 0):,}", 100 * float(mask_info.get('share_replaced', 0))))
        if not masked_run: mask_html += '<p><b>Note:</b> this run\'s embeddings were computed on the unmasked text (the text hash does not match the masked file).</p>'
    emb_keys = ['model', 'revision', 'max_seq_length', 'batch_size', 'n_chunks', 'dim', 'device'] + (['text_file'] if emb_info.get('text_file') else [])
    fit_html = ''
    if dd:
        fit_html = ('<p>Fitted on one representative edition per work: %s chunks of %s editions. The other %s editions (%s chunks) were placed afterwards by <code>umap.transform</code> and <code>hdbscan.approximate_predict</code> and are excluded from the topic statistics and the genre comparison (each chunk page shows whether it was in the fit). '
                    'Edition policy: the earliest dated edition, except the New Oxford Shakespeare control texts for eight Shakespeare works (Folio for seven, Q1 for <i>Lear</i>) — the policy of Kim\'s project, recorded in <code>edition_selection.csv</code>. Among the representative editions %.1f %% of chunks are unassigned.</p>'
                    % (f"{n_fit:,}", len(dd.get('primary_editions', [])), len(set(meta[c]['edition_id'] for c in ids)) - len(dd.get('primary_editions', [])), f"{n_placed:,}", 100 * float(run_info.get('outlier_share_in_fit', run_info.get('outlier_share', 0)))))
    scheme_html = ''
    if scheme_rows:
        scheme_html = ('<h2>Choice of clustering scheme</h2><p>The same embeddings were clustered under several schemes, three seeds each; statistics on the representative editions. ARI = adjusted Rand index between seed pairs (a similarity of partitions, not a share of clusters reproduced); flip = share of chunks that are an outlier in one seed and assigned in another; name share = share of the top-10 keywords that are character names. '
                       'HDBSCAN with min_samples 10 was taken as the main analysis (stable cores, few single-work clusters, coverage reported as a share); K-means 50 assigns every chunk but its partitions vary more between seeds and its cluster edges were found diffuse on reading, so it is kept as a limited comparison for a few core themes, not as a second analysis.</p>'
                       '<table><thead><tr><th>scheme</th><th>method</th><th class="num">clusters</th><th class="num">outliers %</th><th class="num">single-work clusters</th><th class="num">median works / cluster</th><th class="num">largest</th><th class="num">ARI assigned</th><th class="num">ARI all</th><th class="num">flip %</th><th class="num">name share %</th></tr></thead><tbody>'
                       + ''.join(f'<tr><td>{E(r["scheme"])}</td><td>{E(r["method"])}</td><td class="num">{r["clusters"]}</td><td class="num">{r["outliers"]}</td><td class="num">{r["single_work_clusters"]}</td><td class="num">{r["median_works_per_cluster"]}</td><td class="num">{r["largest_cluster"]}</td><td class="num">{r["ari_assigned"]}</td><td class="num">{r["ari_all"]}</td><td class="num">{r["flip_share"]}</td><td class="num">{r["name_share_top10"]}</td></tr>' for r in scheme_rows)
                       + '</tbody></table>')
    st_counts = Counter((sheet.get(t, {}).get('review_status') or 'draft_ai') for t in topics)
    n_notes = sum(1 for t in topics if (sheet.get(t, {}).get('Notes') or '').strip())
    n_read = sum(1 for t in topics if (sheet.get(t, {}).get('chunks_read') or '').strip())
    n_relabel = sum(1 for t in topics if (sheet.get(t, {}).get('Label') or '').strip() and (sheet.get(t, {}).get('Label') or '').strip() != (sheet.get(t, {}).get('draft_label') or '').strip())
    st_label = {'user_confirmed': 'confirmed by the researcher', 'user_pending': 'left pending by the researcher', 'user_labelled': 'relabelled, category unchanged', 'draft_ai': 'AI draft, not yet reviewed', 'checked_ai': 'AI-checked'}
    review_html = ('<h2>Review of the clusters</h2>'
                   '<p><b>Drafts.</b> For every cluster an AI draft was written from the uniform keywords, the work concentration (share of the largest work, of the three largest, number of works with three or more chunks) and a sample of chunks — the chunks read are listed per cluster (<code>chunks_read</code>, %d clusters). '
                   ...
                   % (n_read, len(topics), ', '.join(f'{n} {E(CAT_LABEL.get(c, c))}' for c, n in sorted(use_counts.items(), key=lambda kv: -kv[1])),
                      ', '.join(f'{n} {E(st_label.get(k, k))}' for k, n in sorted(st_counts.items(), key=lambda kv: -kv[1])), n_notes, n_relabel))
    page = (head('Methods') + mast('', 'methods.html') + crumbs('<a href="index.html">Home</a>', 'Methods') + '<h1>Methods</h1>'
            ...
            + mask_html
            + '<h2>Embedding</h2>' + ('<p>Computed on the name-masked chunk text.</p>' if masked_run else '') + '<table>' + kv(emb_info, emb_keys) + '</table>'
            + '<h2>Clustering</h2><p>UMAP to 5 dimensions (n_neighbors 15, min_dist 0.05, cosine) → HDBSCAN (min_cluster_size %s, min_samples %s, Euclidean, EOM). Three seeds were run; this site shows seed %d and reports each chunk\'s topic in the other seeds on its page.</p>' % (hd.get('min_cluster_size', 30), hd.get('min_samples') or 'default', a.seed)
            + fit_html + '<table>' + kv(run_info, ['n_chunks', 'n_in_fit', 'n_topics', 'outlier_share_in_fit', 'outlier_share', 'seed']) + '</table>'
            + scheme_html
